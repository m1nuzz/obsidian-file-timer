...
function create_fragment(ctx) {
  ...
  return {
    ...
    p(ctx2, [dirty]) {
      ...
      if (dirty & 6) {
        startButton.textContent = ctx2[1] && !ctx2[2] ? "Pause" : "Start";
      }
      if (dirty & 8) {
        saveCheckbox.checked = ctx2[3];
      }
    },
    d(detaching) {
...
function instance($$self, $$props, $$invalidate) {
  ...
  function handleReset() {
    plugin.resetTimer(file_path);
  }
  function handleInput() {
    $$invalidate(0, timeString = this.value);
  }
  function handleBlur() {
    const totalSeconds = parseTimeString(timeString);
    plugin.startTimer(file_path, totalSeconds, true, saveTime);
...
var TimerComponent = class extends SvelteComponent {
  constructor(options) {
    super();
    init(this, options, instance, create_fragment, safe_not_equal, { plugin: 3, file_path: 4 });
  }
};
var SoundSuggestModal = class extends import_obsidian.FuzzySuggestModal {
...
var TimerSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Show ribbon icon").setDesc("Toggle the icon in the left ribbon.").addToggle((toggle) => toggle.setValue(this.plugin.settings.showRibbonIcon).onChange(async (value) => {
      this.plugin.settings.showRibbonIcon = value;
      await this.plugin.saveSettings();
      this.plugin.updateRibbonIcon();
    }));
    new import_obsidian.Setting(containerEl).setName("Notification Sound").setDesc("Play a sound when the timer finishes").addToggle((toggle) => toggle.setValue(this.plugin.settings.notificationSound).onChange(async (value) => {
      this.plugin.settings.notificationSound = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Notification Timeout").setDesc("Timeout for notification in seconds. Set to 0 to disable auto-closing.").addText((text) => text.setPlaceholder("0").setValue(String(this.plugin.settings.notificationTimeout)).onChange(async (value) => {
    ...
    volumeSlider.setLimits(0, 100, 1).setValue(this.plugin.settings.volume).onChange(async (value) => {
    ...
    volumeText.setValue(String(this.plugin.settings.volume)).onChange(async () => {
    ...
    new import_obsidian.Setting(containerEl).setName("Custom Sound URL / Path").setDesc("Enter a URL for an audio file, or browse for a local file in your vault. Leave empty to use the default sound.").addText((text) => {
      text.setPlaceholder("https://example.com/sound.mp3 or /sounds/sound.mp3").setValue(this.plugin.settings.customSoundUrl).onChange(async (value) => {
...
var FileTimerPlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    this.timerUpdateCallbacks = /* @__PURE__ */ new Set();
    ...
    for (const path in this.settings.timers) {
      ...
      if (timer.isRunning && !timer.isPaused) {
        ...
        if (timer.remainingTime <= 0) {
          ...
          new import_obsidian.Notice(`Timer finished for ${path}`, this.settings.notificationTimeout * 1e3);
